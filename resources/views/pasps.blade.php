 ...
 <div class="w3-container">
        ...
        <form class="w3-container w3-card-4 w3-theme" method="POST" action="{{ url('pasps')}}">
                ...
                <div class="w3-row w3-section">
                    ...
                    <div class="w3-rest">
                        <input type="date" name="date_zamer" id="pasp-date_zamer" class="w3-input w3-border w3-round" value="{{ old('pasp') }}" required>
                    </div>
                </div>
                                ...
                                <td><!-- Delete Button -->
                                    <div>
                                        <form action="{{ url('pasps/'.$pasp->id) }}" method="POST">
                                                {{ csrf_field() }}
                                                {{ method_field('DELETE') }}
                                                <button type="submit" onclick="return confirm('Дійсно бажаєте вилучити? {{ $pasp->date_zamer }} ')" class="w3-button w3-round w3-red w3-hover-theme">
