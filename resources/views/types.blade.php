                    ...
                    <tbody>
                        @foreach ($types as $type)
                            <tr>
                                ...
                                <td><!-- Delete Button -->
                                    <div>
                                        <form action="{{ url('types/'.$type->id) }}" method="POST">
                                                {{ csrf_field() }}
                                                {{ method_field('DELETE') }}
                                                <button type="submit"  disabled onclick="return confirm('Дійсно бажаєте вилучити? {{ $type->name_type }} ')"class="w3-button w3-round w3-red w3-hover-theme">
                                ...
                                <td><!-- Edit Button -->
                                    <div>    
                                        <a class="w3-button w3-round w3-blue w3-hover-theme" href="{{ route('types.edit',$type->id) }}" disabled><i class="fa fa-edit"></i>Редагувати</a>
                                    </div>       
                                </td>
                            </tr>
                        @endforeach
